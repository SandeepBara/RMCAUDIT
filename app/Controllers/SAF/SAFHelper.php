...
class SAFHelper extends Controller
{
	protected $db;
	protected $dbSystem;
	...
	public function makeBuildingFloorDtlArr_old($inputs) {
		...
		$floorDtlArr = [];
		$sum_of_buildup_area = array_sum($inputs['builtup_area']);
		$forpoint2 = array_intersect($inputs['usage_type_mstr_id'], [2, 4, 13, 17, 20, 22])?1:0;
		...
		for ($i=0; $i<sizeof($inputs['floor_mstr_id']); $i++) {
			...
			if($property_type == "DLX_APARTMENT" && $inputs['const_type_mstr_id'][$i]!=1)
			{
			...
			if ($inputs['usage_type_mstr_id'][$i]==44) {
				...
				if ($sum_of_buildup_area > 25000 && $forpoint2>0) {
	...
	public function checkIsTrustOrTrustSchoolVerified($saf_dtl_id)
	{
		$isTrustOrTrustSchoolVerified = TRUE;
		$sql = "SELECT * FROM tbl_saf_doc_dtl
				WHERE other_doc='trust_document' and verify_status=2 and status=1 and verified_by_emp_id>0 and saf_dtl_id=".$saf_dtl_id;
		$fetch = $this->db->query($sql)->getResultArray();
		
		if(isset($fetch) && sizeof($fetch)>0)
		{
			$isTrustOrTrustSchoolVerified = FALSE;
		}
		return $isTrustOrTrustSchoolVerified;
	}

	public function makeBuildingFloorDtlArr_bk($inputs) {
		//print_var($inputs);
		$ward_no = $this->db->table("view_ward_mstr")
							->select("split_part(split_part(ward_no, 'A', 1), '/', 1) AS ward_no")
							->where("id", $inputs['ward_mstr_id'])
		...
		$floorDtlArr = [];
		//$sum_of_buildup_area = array_sum($inputs['builtup_area']);
		$sum_of_buildup_area = 0;
		$forpoint2 = array_intersect($inputs['usage_type_mstr_id'], [2, 4, 13, 17, 20, 21, 22, 46])?1:0;
		for($i=0; $i<sizeof($inputs['floor_mstr_id']); $i++)
		{
			if(in_array($inputs['usage_type_mstr_id'][$i],[2, 4, 13, 17, 20, 21, 22, 46]))
			{
				$sum_of_buildup_area = $sum_of_buildup_area + $inputs['builtup_area'][$i];
			}
		}
		...
		for ($i=0; $i<sizeof($inputs['floor_mstr_id']); $i++) {
			...
			$sendInput = ['usage_type_mstr_id'=>$usage_type_mstr_id, 'const_type_mstr_id'=>$inputs['const_type_mstr_id'][$i], 'zone_mstr_id'=>$inputs['zone_mstr_id']];
			$mrr_old = $this->model_arr_old_building_mstr->getMRRCalRate($sendInput)['rate'];
			if($inputs['usage_type_mstr_id'][$i]==11){
				$mrr_old=0.00;
			}
			$old_arv = $inputs['builtup_area'][$i]*$mrr_old;
			// IN old rule less some percentage in arv (Residential - 30%, Commercial - 15%)
			...
			$usage_type = ($inputs['usage_type_mstr_id'][$i])=="1"?"RESIDENTIAL":"COMMERCIAL";
			$cvr2022  = $this->model_capital_value_rate->getCVR2022($property_type, $ward_no, $road_type_mstr_id, $usage_type)["rate"];
			$cvr2022_max  = $this->model_capital_value_rate->getCVR2022($property_type, $ward_no, $road_type_mstr_id, $usage_type)["max_rate"];
			$occupancyTypeRate = (float)($inputs['occupancy_type_mstr_id'][$i]==1)?1:1.5;
			//$resiCommTypeRate = (float)($usage_type=="RESIDENTIAL")?0.00075:0.0015;
			$resiCommTypeRate = 0;
			if ($inputs['usage_type_mstr_id'][$i]==44) {
			...
			$cv_mf_old = $cv_mf;
			$cv2022_old = $cvr2022*$inputs['builtup_area'][$i]*$occupancyTypeRate*$resiCommTypeRate*$cv_mf*$matrixFactorRate;
			
			if(isset($inputs['is_trust_school']) || isset($inputs['saf_dtl_id']))
			{
				$isTrust = FALSE;
				$trustType = 0;
				if(isset($inputs['saf_dtl_id']) && $inputs['saf_dtl_id']>0)
				{
					$trustType = $this->model_saf_dtl->getTrustType($inputs['saf_dtl_id']);
					$trustType = $trustType['trust_type']??0;
					$isTrust = $this->checkIsTrustOrTrustSchoolVerified($inputs['saf_dtl_id']);
					
				}else{
					$trustType = $inputs['is_trust_school']??0;
					$isTrust = TRUE;
				}
				
				
				if($isTrust == 1 && in_array($trustType, [1,2]) && !in_array($inputs['usage_type_mstr_id'][$i], [11]))
				{
			...
			$cv2022 = $cvr2022_max*$inputs['builtup_area'][$i]*$occupancyTypeRate*$resiCommTypeRate*$cv_mf*$matrixFactorRate;
			// end capital value calc rate
			...
			$floorDtlArr[] = [
				...
				'old_arv_cal_method'=>['rental_rate'=>$mrr_old, 'buildup_area'=>$inputs['builtup_area'][$i], 'rebate_on_arv'=>$rebate_on_arv],
				'new_arv_cal_method'=>['usage_factor'=>$mf, 'occupancy_factor'=>$afr, 'rental_rate'=>$mrr, 'carper_area'=>$carperArea],
				'cv_2022_cal_method_old'=>['cvr'=>$cvr2022, 'buildup_area'=>$inputs['builtup_area'][$i], 'occupancy_rate'=>$occupancyTypeRate, 'resi_comm_type_rate'=>$resiCommTypeRate, 'calculation_factor'=>$cv_mf_old,'matrix_factor_rate'=>$matrixFactorRate],
				'cv_2022_cal_method'=>['cvr'=>$cvr2022_max, 'buildup_area'=>$inputs['builtup_area'][$i], 'occupancy_rate'=>$occupancyTypeRate, 'resi_comm_type_rate'=>$resiCommTypeRate, 'calculation_factor'=>$cv_mf,'matrix_factor_rate'=>$matrixFactorRate]
			];
			...
			if ( $floorDateUptoFyID<>0 && $floorDateUptoQtr<>0 ) {
				...
				$floorDtlArr[] = [
					...
					'old_arv_cal_method'=>['rental_rate'=>$mrr_old, 'buildup_area'=>$inputs['builtup_area'][$i], 'rebate_on_arv'=>$rebate_on_arv],
					'new_arv_cal_method'=>['usage_factor'=>$mf, 'occupancy_factor'=>$afr, 'rental_rate'=>$mrr, 'carper_area'=>$carperArea],
					'cv_2022_cal_method_old'=>['cvr'=>$cvr2022, 'buildup_area'=>$inputs['builtup_area'][$i], 'occupancy_rate'=>$occupancyTypeRate, 'resi_comm_type_rate'=>$resiCommTypeRate, 'calculation_factor'=>$cv_mf, 'matrix_factor_rate'=>$matrixFactorRate],
					'cv_2022_cal_method'=>['cvr'=>$cvr2022_max, 'buildup_area'=>$inputs['builtup_area'][$i], 'occupancy_rate'=>$occupancyTypeRate, 'resi_comm_type_rate'=>$resiCommTypeRate, 'calculation_factor'=>$cv_mf,'matrix_factor_rate'=>$matrixFactorRate]
				];
			}
		}
		...
		if ($inputs['is_mobile_tower']==1) {
			...
			$resiCommTypeRate = (float)0.0015;
			$matrixFactorRate = 1;
			$cv2022_old = $cvr2022*$inputs['tower_area']*$occupancyTypeRate*$resiCommTypeRate*$matrixFactorRate;
			$cv2022 = $cvr2022_max*$inputs['tower_area']*$occupancyTypeRate*$resiCommTypeRate*$matrixFactorRate;
			...
			$floorDtlArr[] = [
				...
				'old_arv_cal_method'=>null,
				'new_arv_cal_method'=>['usage_factor'=>1.5, 'occupancy_factor'=>1.5, 'rental_rate'=>$mrr, 'carper_area'=>$carperArea],
				'cv_2022_cal_method_old'=>['cvr'=>$cvr2022, 'buildup_area'=>$inputs['tower_area'], 'occupancy_rate'=>$occupancyTypeRate, 'resi_comm_type_rate'=>$resiCommTypeRate, 'matrix_factor_rate'=>$matrixFactorRate],
				'cv_2022_cal_method'=>['cvr'=>$cvr2022_max, 'buildup_area'=>$inputs['tower_area'], 'occupancy_rate'=>$occupancyTypeRate, 'resi_comm_type_rate'=>$resiCommTypeRate, 'matrix_factor_rate'=>$matrixFactorRate]
			];
		}
		...
		if ($inputs['is_hoarding_board']==1) {
			...
			$resiCommTypeRate = (float)0.0015;
			$matrixFactorRate = 1;
			$cv2022_old = $cvr2022*$inputs['hoarding_area']*$occupancyTypeRate*$resiCommTypeRate*$matrixFactorRate;
			$cv2022 = $cvr2022_max*$inputs['hoarding_area']*$occupancyTypeRate*$resiCommTypeRate*$matrixFactorRate;
			// end capital value calc rate
			...
			$floorDtlArr[] = [
				...
				'old_arv_cal_method'=>null,
				'new_arv_cal_method'=>['usage_factor'=>1.5, 'occupancy_factor'=>1.5, 'rental_rate'=>$mrr, 'carper_area'=>$carperArea],
				'cv_2022_cal_method_old'=>['cvr'=>$cvr2022, 'buildup_area'=>$inputs['hoarding_area'], 'occupancy_rate'=>$occupancyTypeRate, 'resi_comm_type_rate'=>$resiCommTypeRate, 'matrix_factor_rate'=>$matrixFactorRate],
				'cv_2022_cal_method'=>['cvr'=>$cvr2022_max, 'buildup_area'=>$inputs['tower_area'], 'occupancy_rate'=>$occupancyTypeRate, 'resi_comm_type_rate'=>$resiCommTypeRate, 'matrix_factor_rate'=>$matrixFactorRate]
			];
		}
		...
		if($inputs['is_petrol_pump']==1 && $inputs['prop_type_mstr_id']!=4){
			...
			$resiCommTypeRate = (float)0.0015;
			$matrixFactorRate = 1;
			$cv2022_old = $cvr2022*$inputs['under_ground_area']*$occupancyTypeRate*$resiCommTypeRate*$matrixFactorRate;
			$cv2022 = $cvr2022_max*$inputs['under_ground_area']*$occupancyTypeRate*$resiCommTypeRate*$matrixFactorRate;
			// end capital value calc rate
			...
			$floorDtlArr[] = [
				...
				'old_arv_cal_method'=>null,
				'new_arv_cal_method'=>['usage_factor'=>1.5, 'occupancy_factor'=>1.5, 'rental_rate'=>$mrr, 'carper_area'=>$carperArea],
				'cv_2022_cal_method_old'=>['cvr'=>$cvr2022, 'buildup_area'=>$inputs['under_ground_area'], 'occupancy_rate'=>$occupancyTypeRate, 'resi_comm_type_rate'=>$resiCommTypeRate, 'matrix_factor_rate'=>$matrixFactorRate],
				'cv_2022_cal_method'=>['cvr'=>$cvr2022_max, 'buildup_area'=>$inputs['under_ground_area'], 'occupancy_rate'=>$occupancyTypeRate, 'resi_comm_type_rate'=>$resiCommTypeRate, 'matrix_factor_rate'=>$matrixFactorRate]
			];
		}
	...
	public function makeBuildingFloorDtlArr($inputs) {
		$ward_no = $this->db->table("view_ward_mstr")
							...
							->get()
            				->getFirstRow("array")["ward_no"];
        $new_ward_no = $this->db->table("view_ward_mstr")
            ->select("split_part(split_part(ward_no, 'A', 1), '/', 1) AS ward_no")
            // ->select("ward_no")
            ->where("id", $inputs['new_ward_mstr_id'])
            ->get()
            ->getFirstRow("array")["ward_no"];
		$taxEffectedFrom = date('Y-04-01', strtotime('-12 year'));
        $taxEffectedFromFY = getFY($taxEffectedFrom);
		...
		for ($i=0; $i<sizeof($inputs['floor_mstr_id']); $i++) {
            ...
            $cvr2024  = $this->model_capital_value_rate->getCVR2024($property_type, $new_ward_no, $road_type_mstr_id, $usage_type)["rate"]??0;
            $cvr2024_max  = $this->model_capital_value_rate->getCVR2024($property_type, $new_ward_no, $road_type_mstr_id, $usage_type)["max_rate"];
            $occupancyTypeRate = (float)($inputs['occupancy_type_mstr_id'][$i]==1)?1:1.5;
			//$resiCommTypeRate = (float)($usage_type=="RESIDENTIAL")?0.00075:0.0015;
			$resiCommTypeRate = 0;
			if ($inputs['usage_type_mstr_id'][$i]==44) {
			...
			$cv2022 = $cvr2022_max*$inputs['builtup_area'][$i]*$occupancyTypeRate*$resiCommTypeRate*$cv_mf*$matrixFactorRate;
            $cv2024 = $cvr2024_max*$inputs['builtup_area'][$i]*$occupancyTypeRate*$resiCommTypeRate*$cv_mf*$matrixFactorRate;  #todo
			if(isset($inputs["is_gb_saf"]) && isset($inputs["comperitiv_tax"]) && isset($inputs["prop_id"])  && in_array($inputs["prop_id"],[242,239,436]))
			{
				$resiCommTypeRate = $sum_of_buildup_area > 25000 ? (float)0.0015 : $resiCommTypeRate;
				$resiCommTypeRate = in_array($inputs['builtup_area'][$i],[596400,817256]) ? 0.00075 : $resiCommTypeRate;
				if(in_array($inputs["prop_id"],[436])){
					$resiCommTypeRate = (round($inputs['builtup_area'][$i])<(31542.20)) ? 0.00075 : $resiCommTypeRate;
					$resiCommTypeRate = in_array($inputs['builtup_area'][$i],[71135.00,59468.61,58272.62,51318.00,4028.90]) ? 0.0020 : $resiCommTypeRate;
				}
				$cv2022 = $cvr2022_max*$inputs['builtup_area'][$i]*$occupancyTypeRate*$resiCommTypeRate*$cv_mf*$matrixFactorRate;
			}
			// end capital value calc rate
			...
			$floorDtlArr[] = [
				...
				'cv_2022_cal_method_old'=>['cvr'=>$cvr2022, 'buildup_area'=>$inputs['builtup_area'][$i], 'occupancy_rate'=>$occupancyTypeRate, 'resi_comm_type_rate'=>$resiCommTypeRate, 'calculation_factor'=>$cv_mf_old,'matrix_factor_rate'=>$matrixFactorRate],
				'cv_2022_cal_method'=>['cvr'=>$cvr2022_max, 'buildup_area'=>$inputs['builtup_area'][$i], 'occupancy_rate'=>$occupancyTypeRate, 'resi_comm_type_rate'=>$resiCommTypeRate, 'calculation_factor'=>$cv_mf,'matrix_factor_rate'=>$matrixFactorRate],
				'cv_2024_cal_method'=>['cvr'=>$cvr2024_max, 'buildup_area'=>$inputs['builtup_area'][$i], 'occupancy_rate'=>$occupancyTypeRate, 'resi_comm_type_rate'=>$resiCommTypeRate, 'calculation_factor'=>$cv_mf,'matrix_factor_rate'=>$matrixFactorRate]
			];
			if ( $floorDateUptoFyID<>0 && $floorDateUptoQtr<>0 ) {
				if ( $floorDateUptoQtr==4 ) {
					$floorDateUptoQtr = 1;
		...
		if ($inputs['is_mobile_tower']==1) {
            ...
            $cvr2024  = $this->model_capital_value_rate->getCVR2024($property_type, $new_ward_no, $road_type_mstr_id, $usage_type)["rate"];
            $cvr2024_max  = $this->model_capital_value_rate->getCVR2024($property_type, $new_ward_no, $road_type_mstr_id, $usage_type)["max_rate"];
			...
			$floorDtlArr[] = [
				...
				'cv_2022_cal_method_old'=>['cvr'=>$cvr2022, 'buildup_area'=>$inputs['tower_area'], 'occupancy_rate'=>$occupancyTypeRate, 'resi_comm_type_rate'=>$resiCommTypeRate, 'matrix_factor_rate'=>$matrixFactorRate],
				'cv_2022_cal_method'=>['cvr'=>$cvr2022_max, 'buildup_area'=>$inputs['tower_area'], 'occupancy_rate'=>$occupancyTypeRate, 'resi_comm_type_rate'=>$resiCommTypeRate, 'matrix_factor_rate'=>$matrixFactorRate],
                'cv_2024_cal_method'=>['cvr'=>$cvr2024_max, 'buildup_area'=>$inputs['tower_area'], 'occupancy_rate'=>$occupancyTypeRate, 'resi_comm_type_rate'=>$resiCommTypeRate, 'matrix_factor_rate'=>$matrixFactorRate] #todo
			];
		}
		...
		if ($inputs['is_hoarding_board']==1) {
			...
			$cv2024 = $cvr2024_max*$inputs['hoarding_area']*$occupancyTypeRate*$resiCommTypeRate*$matrixFactorRate;
			// end capital value calc rate
			...
			$floorDtlArr[] = [
				...
				'new_arv_cal_method'=>['usage_factor'=>1.5, 'occupancy_factor'=>1.5, 'rental_rate'=>$mrr, 'carper_area'=>$carperArea],
				'cv_2022_cal_method_old'=>['cvr'=>$cvr2022, 'buildup_area'=>$inputs['hoarding_area'], 'occupancy_rate'=>$occupancyTypeRate, 'resi_comm_type_rate'=>$resiCommTypeRate, 'matrix_factor_rate'=>$matrixFactorRate],
				'cv_2022_cal_method'=>['cvr'=>$cvr2022_max, 'buildup_area'=>$inputs['hoarding_area'], 'occupancy_rate'=>$occupancyTypeRate, 'resi_comm_type_rate'=>$resiCommTypeRate, 'matrix_factor_rate'=>$matrixFactorRate],
                'cv_2024_cal_method'=>['cvr'=>$cvr2024_max, 'buildup_area'=>$inputs['hoarding_area'], 'occupancy_rate'=>$occupancyTypeRate, 'resi_comm_type_rate'=>$resiCommTypeRate, 'matrix_factor_rate'=>$matrixFactorRate] #todo
			];
		}
		...
		if($inputs['is_petrol_pump']==1 && $inputs['prop_type_mstr_id']!=4){
            ...
            $cv2024 = $cvr2024_max*$inputs['under_ground_area']*$occupancyTypeRate*$resiCommTypeRate*$matrixFactorRate;
			// end capital value calc rate
			...
			$floorDtlArr[] = [
				...
				'cv_2022_cal_method_old'=>['cvr'=>$cvr2022, 'buildup_area'=>$inputs['under_ground_area'], 'occupancy_rate'=>$occupancyTypeRate, 'resi_comm_type_rate'=>$resiCommTypeRate, 'matrix_factor_rate'=>$matrixFactorRate],
				'cv_2022_cal_method'=>['cvr'=>$cvr2022_max, 'buildup_area'=>$inputs['under_ground_area'], 'occupancy_rate'=>$occupancyTypeRate, 'resi_comm_type_rate'=>$resiCommTypeRate, 'matrix_factor_rate'=>$matrixFactorRate],
                'cv_2024_cal_method'=>['cvr'=>$cvr2024_max, 'buildup_area'=>$inputs['under_ground_area'], 'occupancy_rate'=>$occupancyTypeRate, 'resi_comm_type_rate'=>$resiCommTypeRate, 'matrix_factor_rate'=>$matrixFactorRate]
			];
		}
	...
	public function calBuildingTaxDtl_($floorDtlArr, $prop_type_mstr_id, $isAdditionaTaxImplemented) {
		$old_rule_arv_sub = [];
        $new_rule_arv_sub = [];
		$cv_rule_arv_sub = [];
		$vacantLandAdded = 0;
...
//        dd($safTaxDtl,$old_rule_arv_sub,$cv_rule_arv_sub24);
	...
	public function saveSafData($inputs) {
		...
		// $increment = $count+1;
		// $serial = str_pad($increment, 5, "0", STR_PAD_LEFT) . "";
		
		// $saf_no = "SAF"."/".$assessmentId."/".str_pad($wardNo, 3, 0, STR_PAD_LEFT)."/".$serial;//."/".rand(100,999);
		...
		$input = [
			...
			'status'=>1,
			'trust_type'=>!empty($inputs['is_trust_school'])?$inputs['is_trust_school']:null,
			'trade_license_no' => $inputs['trade_license_no'] ?? null
		];
		// echo "<br/>";
		// $d=false;
		// print($d);
		...
		$saf_no = "SAF"."/".$assessmentId."/".str_pad($wardNo, 3, 0, STR_PAD_LEFT)."/".$serial;//."/".rand(100,999);
		while($this->db->table('tbl_saf_dtl')->select('count(id) as count')->where("saf_no",$saf_no)->get()->getFirstRow('array')['count']>0){
			$increment += 1;
			$serial = str_pad($increment, 5, "0", STR_PAD_LEFT) . "";
			$saf_no = "SAF" . "/" . $assessmentId . "/" . str_pad($wardNo, 3, 0, STR_PAD_LEFT) . "/" . $serial;
		}
		if($saf_dtl_id)
		{
	...
	public function calGovtSafDemand($govtSafTaxDtl, $govt_saf_dtl_id) {
		...
		foreach ($govtSafTaxDtl AS $key=>$taxDtl) {
			...
			// tax insert code 
				...
				$fy_mstr_id = $this->model_fy_mstr->getFyByFy(['fy' => $taxDtl['fyear']])['id'];
				//$saf_tax_id = $this->model_saf_tax->insertData($input);
				$sql = "INSERT INTO tbl_govt_saf_tax_dtl (govt_saf_dtl_id, fy_mstr_id, fyear, qtr, arv, holding_tax, water_tax, education_cess, health_cess, latrine_tax, additional_tax, quarterly_tax, created_on, status)
				VALUES ('$govt_saf_dtl_id', '".$fy_mstr_id."', '".$taxDtl['fyear']."', '".$taxDtl['qtr']."', '$arv', '$holding_tax', '$water_tax', '$education_cess', '$health_cess', '$latrine_tax', '$additional_tax', '$quarterly_tax', '".date("Y-m-d H:i:s")."', 1) returning id";
				$query = $this->db->query($sql);
				$return = $query->getFirstRow("array");
	...
	public function calDiffSafDemand($safTaxDtl, $saf_dtl_id, $prop_dtl_id) {
		...
		foreach ($safTaxDtl AS $key=>$taxDtl) {
			...
			// tax insert code
				...
				$fy_id = $this->getFyID($taxDtl['fyear']);
				...
				VALUES ('$prop_dtl_id', '".$fy_id."', '".$taxDtl['fyear']."', '".$taxDtl['qtr']."', '$arv', '$holding_tax', '$water_tax', '$education_cess', '$health_cess', '$latrine_tax', '$additional_tax', '".date("Y-m-d H:i:s")."', 1, '$quarterly_tax') returning id";
				$query = $this->db->query($sql);
				$return = $query->getFirstRow("array");
