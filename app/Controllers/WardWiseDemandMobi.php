...
class WardWiseDemandMobi extends MobiController
{
    protected $db;
    protected $dbSystem;    
    ...
    public function report()
    {
        ...
        if($this->request->getMethod()=='post')
        {
            ...
            if($data['ward_mstr_id']!=""){
              $demandDtl[0]['demand'] = $this->model_prop_demand->getDemandDetails($data['ward_mstr_id']);
              ...
              $data['demandDtl'] = $demandDtl;
                ...
                foreach ($data['wardList'] as $key => $value) {
                    $demandDtl[$key]['demand'] = $this->model_prop_demand->getDemandDetails($value['id']);
                    $demandDtl[$key]['ward_no'] = $value['ward_no'];
                    $data['total'] = $data['total']+$demandDtl[$key]['demand'];
                }
                $data['demandDtl'] = $demandDtl;
            }
            return view('report/ward_wise_demand_mobi',$data);
        }
        else
        {
            foreach ($data['wardList'] as $key => $value) {
