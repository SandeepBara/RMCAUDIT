            ...
            if($tradeChequeDetails = $this->TradeTransactionModel->getChequeDetailsByEmpId($id,$data['date_from'])){
               // print_r($tradeChequeDetails);
                ...
                foreach ($tradeChequeDetails as $value) {
                    $chequeDetails[$key]['transaction_id'] = $value['id'].'/'.'trade'.'/'.$value['paid_amount'];
                    $chequeDetails[$key]['tran_date'] = $value['transaction_date'];
                    $chequeDetails[$key]['tran_no'] = $value['transaction_no'];
                    $chequeDetails[$key]['payable_amt'] = $value['paid_amount'];
        ...
        if($this->request->getMethod()=='post'){
            ...
            if($len>0){
                ...
                for($i=0;$i<$len;$i++){
                    ...
                    $totalChequeverified =$totalChequeverified+$iparr[2]; 
                    

                    if($iparr[1]=="property"){ 
                        //update property transaction
                        $this->model_transaction->updateVerificationStatus($id,$iparr[0],$verified_date,$cash_verify_id);
                    }else if($iparr[1]=="water"){
                        //Water Transaction
                        $this->Water_Transaction_Model->updateVerificationStatus($id,$iparr[0],$verified_date,$cash_verify_id);
                    }else if($iparr[1]=="trade"){
                        //trade Transaction
                         $this->TradeTransactionModel->updateVerificationStatus($id,$iparr[0],$verified_date,$cash_verify_id);
                    }
                }
            }
            //Cash verified
           if($inputs['cashVerify']=="cashVerify"){
            ...
            $verifiedCash = $inputs['cash_amount'];
                $this->model_transaction->updateVerificationStatuCashCollection($employee_id,$verified_date,$id,$trans_date,$cash_verify_id);
                $this->Water_Transaction_Model->updateVerificationStatuCashCollection($employee_id,$verified_date,$id,$trans_date,$cash_verify_id);
                $this->TradeTransactionModel->updateVerificationStatuCashCollection($employee_id,$verified_date,$id,$trans_date,$cash_verify_id);
            }
