...
class WaterDocument extends AlphaController
{
    ...
    public function doc_upload($id=null){
        ...
        $data['apt_doc']=$this->model_applicant_doc->check_other_doc($apply_connection_id,"Occupancy Certificate");
        ...
        if($this->request->getMethod()=='post'){
            ...
            if(isset($_POST['btn_apt']))
            {
                $data = [
                        'apply_connection_id' => $apply_connection_id,
                        'doc_for' => 'Occupancy Certificate',
                        'document_id' => 7,
                        'emp_details_id' => $login_emp_details_id,
                        'created_on' =>date('Y-m-d H:i:s')
                    ];
                $rules=[
                        'apt_doc_path'=>'uploaded[apt_doc_path]|max_size[apt_doc_path,10240]|ext_in[apt_doc_path,pdf]',
                ];
                if($this->validate($rules)){ 
                    if($apt_doc_last_id = $this->model_applicant_doc->insertData($data)){
                        $apt_doc_file=$this->request->getFile('apt_doc_path');
                        if($apt_doc_file->IsValid() && !$apt_doc_file->hasMoved()){
                             $newFileName = md5($apt_doc_last_id);
                            $file_ext = $apt_doc_file->getExtension();
                            $path = $ulb_city_nm."/"."water_doc_dtl";

                            if($apt_doc_file->move(WRITEPATH.'uploads/'.$path.'/',$newFileName.'.'.$file_ext)){
                                $apt_doc_path = $path."/".$newFileName.'.'.$file_ext;
                                if($updaterow = $this->model_applicant_doc->updatedocpathById($apt_doc_last_id,$apt_doc_path))
                                {
                                    return $this->response->redirect(base_url('waterdocument/doc_upload/'.$id.''));
                                }
                            }
