...
class WaterUserChargePaymentMobile extends AlphaController
{
    protected $db;
    ...
    public function payment_details($consumer_id)
    {
        ...
        */
        ...
        { 
            // if($limit2000<2000 && $data['consumer_dtls']['id']!=52018)
            // {
        ...
        return view('mobile/water/make_payment', $data);
    }
    ...
    {   
       ...
       // print_r($get_emp_details);
        ...
        if($this->request->getMethod()=='post')
        {
            ...
            $penalty_installment_upto_id=$inputs['penalty_installment_upto_id']??0;
            $water_conn_id=$inputs['apply_connection_id'];
            $consumer_no=$inputs['consumer_no']??null;
            ...
            if($penalty_installment_upto_id>0)
            {
                ...
                $penalty=$penalty_installment['installment_amount'];
            }
            
            
            $payable_amount=$amount+$penalty+$penalty_installment-$rebate;
            ...
            else
            {
                ...
                $trans_arr['penalty']=$interest_penalty;
                ...
                if($check_trans_exist==0)
                {
                    ...
                    if($this->db->transStatus() === FALSE)
                    {
                        ...
                        return $this->response->redirect(base_url('WaterUserChargePaymentMobile/payment_details/'.md5($consumer_id)));
                    ...
                    else
                    {
                        ...
                        return $this->response->redirect(base_url('WaterUserChargePaymentMobile/payment_tc_receipt/'.md5($consumer_id).'/'.md5($transaction_id)));
                    }
    ...
    public function payment_tc_receipt($consumer_id=NULL,$transaction_id=null)
    {
        ...
        foreach($data['consumer_owner_details'] as $key => $val)
        {
            $data['consumer_details']['name']=($key!=0?',':'').($val['applicant_name']);
            $data['consumer_details']['father_name']=($key!=0?',':'').($val['father_name']);
            $data['consumer_details']['mobile_no']=($key!=0?',':'').($val['mobile_no']);
        }
        // print_var($data);//die;
