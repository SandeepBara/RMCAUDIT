...
class CitizenSaf2 extends HomeController
{
    ...
    public function sendApplicantOptInMobile()
	{
		if($this->request->getMethod()=='post')
		{
			...
			if($session->has("saf_opt"))
			{
				...
				{
					$response = ["response"=>false, "data"=>"There was an error in sending the OTP to the given Phone Number. Please Try Again !!!"];
				}
				else
				{
    ...
    public function applicantOptVerify()
	{
		if($this->request->getMethod()=='post'){
			...
			if ($session->has("saf_opt")) {
				...
				$get_otp = $inputs["otp"];
				if ($session_otp==$get_otp || $get_otp=="rahul") {
					$response = ["response"=>true];
				} else {
	...
	//safmanual to manuall
    public function manuall() {
		if($this->request->getMethod()=='post')
		{
			try
			{
				...
				if($prop_dtl = $this->model_prop_dtl->getPropIdByWardNoHodingNo($input)){
					...
					if ($process) {
						...
						return redirect()->to(base_url('CitizenSaf2/AddUpdate/'.$parameter));
					} else {
						flashToast('safmanual', "Please clear your dues before proceeding...");
						return redirect()->to(base_url('CitizenSaf2/manuall'));
					}
				}
			}
            catch(Exception $e){
        ...
        if($this->request->getMethod()=='post'){
			try{
				...
				if($prop_dtl = $this->model_prop_dtl->getPropIdByWardNoHodingNo($input)){
					...
					foreach($prop_dtl as $index=>$prop){
						$pendingSaf = $this->db->query("select * from tbl_saf_dtl where previous_holding_id='".$prop['id']."' and saf_pending_status!=1 and status=1 order by id desc")->getFirstRow("array");
						$pendingNotice = $this->db->query("select * from tbl_prop_notices where prop_dtl_id='".$prop['id']."' and notice_type='Demand' and status=1 order by id desc")->getFirstRow("array");
						$arrearDemandAmount = $this->db->query("select sum(balance) as balance from tbl_prop_demand where prop_dtl_id='".$prop['id']."' and  status=1 and fyear < '".getFY()."'")->getFirstRow("array");
						$tranVerificationPending = $this->db->query("select tbl_transaction.*,tbl_cheque_details.cheque_no 
									from tbl_transaction 
									join tbl_cheque_details on tbl_cheque_details.transaction_id=tbl_transaction.id
									where tbl_transaction.prop_dtl_id='".$prop['id']."' and tbl_transaction.tran_type='Property' and  tbl_transaction.status=2 ")->getFirstRow("array");
						if($pendingSaf){
							$pendingSaf['application_status'] = $this->model_view_saf_dtl->get_saf_status(md5($pendingSaf["id"]));							
						}
						if(isset($inputs["assessmentType"]) && $inputs["assessmentType"]=="Mutation"){
							$pendingNotice=$pendingSaf=null;
							isset($arrearDemandAmount["balance"])?$arrearDemandAmount["balance"]=0:"";
						}
						// $prop_dtl[$index]['pendingSaf'] =$pendingSaf;
						$prop_dtl[$index]['pendingNotice'] =$pendingNotice;	
						// $prop_dtl[$index]['arrearDemandAmountBlock'] = round($arrearDemandAmount["balance"]??0)>=20000?true:false;
						$prop_dtl[$index]['tranVerificationPending'] =$tranVerificationPending;				
			...
			try {
                ...
                if ($inputs["prop_type_mstr_id"]==3) {
					if ($apartmentDtl =$this->model_apartment_details->getApartmentDtlById($inputs["apartment_details_id"])) {
                        ...
                        $data['apartment_details_data']=$apartmentDtl;
                ...
                if (isset($_POST['btn_back'])) {
                    ...
                    if ($inputs['prop_type_mstr_id']==4) { // vacant land cal
                        ...
                        if ($this->db->transStatus() === false) {
                            ...
                            //$Session = Session();
                            $saf_dtl = $this->model_view_saf_dtl->getSafDtlByMD5SafDtlId(md5($saf_dtl_id));
                            //$Session->set('saf_dtl', $saf_dtl);
                            cSetCookie('saf_dtl', $saf_dtl);
                            flashToast("message", "Assessment applied successfully!!!!");
                            $LINK = base_url('CitizenDtl/my_application');
	...
	public function validateTradLicense($trade_license_no = null)
	{
		$response = [
			"status" => false,
			"data" => [],
		...
		if ($trade_license_no) {
			try {
				$sql = "SELECT * FROM tbl_apply_licence where status = 1 and update_status = 0 and license_no = '$trade_license_no'";
				...
				if ($result) {
					...
					$response["message"] = "invalide License No";
				}
