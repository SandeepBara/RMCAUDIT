...
class WaterApplyNewConnectionView extends AlphaController
{
    protected $db;
    ...
    public function index()
    {
        ...
        $data['user_type']=$get_emp_details['user_type_mstr_id'];
        ...
        if($this->request->getMethod()=='post')
        {
            ...
            if($this->request->getVar('submit')=="update_application")
            {
               // echo $water_conn_id;
                ...
                //echo $data['connection_dtls']['connection_type_id'];
               ...
               // $data['owner_details']=$this->apply_wtrconn_model->water_owner_details($water_conn_id);
                ...
                return $this->response->redirect(base_url('WaterUpdateApplicationNew/index/'.($water_conn_id)));
            ...
            else if($this->request->getVar('submit')=="upload_documents")
            {
                  ...
                  return $this->response->redirect(base_url('WaterDocument/doc_upload/'.($water_conn_id)));
                  
            }
            else if($this->request->getVar('submit')=="view_documents")
            {
                return $this->response->redirect(base_url('waterdocument/docview/'.($water_conn_id)));
                  
            }
            else if($this->request->getVar('submit')=="proceed_payment")
            {

                 return $this->response->redirect(base_url('WaterPayment/payment/'.($water_conn_id)));
            ...
            else if($this->request->getVar('submit')=="view_connection_fee")
            {
                ...
                //$data['conn_fee_charge_details']=$this->conn_view_model->conn_fee_charge_details($water_conn_id);

              //  $data['conn_fee_charge']=$this->conn_view_model->conn_fee_charge_details($water_conn_id);

          
                   return $this->response->redirect(base_url('WaterViewConnectionCharge/fee_charge/'.($water_conn_id)));
