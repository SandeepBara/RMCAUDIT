...
class WaterApplyNewConnectionMobi extends MobiController
{
    ...
    public function __construct()
    {
        ...
        parent::__construct();
        helper(['db_helper', 'form','sms_helper','form_helper']);
    ...
    public function index()
    {
        ...
        $data['ulb_mstr_id']=$this->get_ulb_detail['ulb_mstr_id'];
        ...
        if($this->request->getMethod()=='post')
        {
            if($this->request->getVar('id')=="")
            {
              ...
              */
                ...
                if($data['prop_id']>0)
                {
                    ...
                    // if($count_prop>0 and $data['owner_type']=='OWNER') // in case multiple connection apply from same holding owner can apply only once but for now only one connection can be given on one holding so 
             ...
             */
                ...
                $apply_water_conn['apply_from']='TC';
                ...
                else
                {
                    ...
                    if($insert_id)
                    {
                        ...
                        if($this->user_type && $this->user_type==5)
                        {
                            $consumer_details["id"]=$insert_id;
                            $consumer_details["application_no"]=$app_no;
                            $vistingRepostInput = waterConnApplyVisit($consumer_details,$this->request->getVar());           
                        ...
                        SMSJHGOVT($owner_arr['mobile_no'], $sms, "1307161908260898924");
    ...
    public function water_connection_view($insert_id)
    {
            ...
            $data['penalty_installment']=$this->penalty_installment_model->getUnpaidInstallment($insert_id);
            $data['pay']=false;
            if(!empty($data['penalty_installment']))
            {
                $data['pay']=true;
            ...
            return view('mobile/water/water_connection_details_view',$data);
