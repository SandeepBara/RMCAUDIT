...
class WaterSummaryReport extends AlphaController
{   
    protected $db;
    protected $dbSystem;
    protected $ulb_id;
    ...
    {   
        ...
        {   
            ...
            $where_billed=" generation_date between '".$data['date_from']."' and '".$data['date_upto']."'";
            $where_approved=" and forward_date between '".$data['date_from']."' and '".$data['date_upto']."'";
            $where_payment=" and transaction_date between '".$data['date_from']."' and '".$data['date_upto']."'";
            
            $data['billed_count']=$this->water_report_model->countDemandBilled($where_billed);      
            $data['approved_count']=$this->water_report_model->countApproved($where_approved);      
            $data['payment_count']=$this->water_report_model->countPayment($where_payment);      
            
            if($inputs['approved'])
            {
                $data['heading']='Approved List';
                $data['result']=$this->water_report_model->approvedList($where_approved);

            }
            else if($inputs['billed'])
            {
                $data['heading']='Billed List';
                $data['result']=$this->water_report_model->billedList($where_billed);
            }
            else if($inputs['payment'])
            {
                $data['heading']='Payment List';
                $data['result']=$this->water_report_model->paymentList($where_payment);
            }
        ...
        return view('water/report/summary_report',$data);
