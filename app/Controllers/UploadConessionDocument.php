...
class UploadConessionDocument extends AlphaController
{
    protected $db;
    protected $dbSystem;
    ...
    public function prop_inbox_list()
    {
        ...
        if (isset($data["search_key"])) {
            $session->set('search_key', $data["search_key"]);
        }
        ...
        $where1 ="";
        if(!empty($search_key)){
            $where =" where (holding_no='" . $search_key . "' OR new_holding_no='" . $search_key . "')";
        }else{
            $where1 =" and (date(created_on) >='" . $from_date . "' AND date(created_on) <='" . $to_date . "')";
        }
        ...
        $sql = "SELECT 
                ...
                from tbl_prop_dtl
                inner join (
                    select distinct(prop_dtl_id) from tbl_prop_doc_special_dtl 
                    where verify_status=0 and status=1 and doc_path='' $where1
                ) prop_doc_dtl on tbl_prop_dtl.id=prop_doc_dtl.prop_dtl_id 
                inner join view_ward_mstr on tbl_prop_dtl.ward_mstr_id=view_ward_mstr.id ". $where;
        ...
        return view('property/saf/consession_doc_upload', $data);
    }
    ...
    public function prop_inbox_index()
    {
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            // $where = "tbl_prop_dtl.ward_mstr_id in (" . implode(",", $ward) . ") AND date(tbl_level_doc_verify_dtl.created_on) >='" . $data['from_date'] . "' 
            // AND date(tbl_level_doc_verify_dtl.created_on) <='" . $data['to_date'] . "' and tbl_prop_doc_special_dtl.verify_status='0' AND  tbl_level_doc_verify_dtl.status='1' group by tbl_prop_dtl.id,tbl_level_doc_verify_dtl.id  ORDER BY tbl_level_doc_verify_dtl.id DESC";
            $where1 = " date(created_on) >='" . $data['from_date'] . "' 
			AND date(created_on) <='" . $data['to_date'] . "'";
            $where2 = " tbl_prop_dtl.ward_mstr_id in (" . implode(",", $ward) . ")";

            // if($data['holding_no']!=''){
            //     $where2 = " tbl_prop_dtl.ward_mstr_id in (" . implode(",", $ward) . ") and holding_no=".$data['holding_no']." or new_holding_no=".$data['holding_no']." ";
            // }
        } else {
            $data['holding_no'] = "";
            $data['from_date'] = date('Y-m-d');
        ...
        // $session->set('where', $where);
        $session->set('where1', $where1);
        $session->set('where2', $where2);
        // $session->set('holding_no', $data['holding_no']);
        $session->set('from_date', $data['from_date']);
        $session->set('to_date', $data['to_date']);
        return $this->response->redirect(base_url('UploadConessionDocument/prop_inbox_list/'));
    }
    ...
    public function list_of_saf_generated_memo()
    {
        ...
        $sql = "SELECT
                ...
                cast(tbl_saf_memo_dtl.created_on as date) between '$from_date' and '$to_date' and tbl_saf_dtl.ward_mstr_id in ($fromWard)";
    ...
    public function verifyDocument($saf_dtl_id_MD5 = NULL)
    {
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            if (isset($_POST['btn_generate_memo'])) {
                ...
                $ward_no = $this->model_ward_mstr->getdatabyid($saf_dtl['new_ward_mstr_id'] ?? $saf_dtl['ward_mstr_id'])['ward_no'];
                $ward_no = str_pad($ward_no, 3, "0", STR_PAD_LEFT);
                ...
                if ($saf_dtl['assessment_type'] == "Reassessment") {
                    ...
                    if ($this->model_prop_dtl->updatePropDetByDocVerifyTime($input)) {
                        ...
                        foreach ($saf_tax_list as $key => $saf_tax) {
                            $saf_tax_details = $saf_tax;
                            $saf_tax_id = $saf_tax['id'];
                            $prop_tax_id = $this->model_prop_tax->insertpropaxdetbysafid($prop_dtl_id, $saf_tax['fy_mstr_id'], $saf_tax['qtr'], $saf_tax['arv'], $saf_tax['holding_tax'], $saf_tax['water_tax'], $saf_tax['education_cess'], $saf_tax['health_cess'], $saf_tax['latrine_tax'], $saf_tax['additional_tax'], $created_on, $saf_tax['fyear']);
                            $this->model_prop_demand->selectInsertBySafDtlAndSafTaxDtlId($level_dtl['saf_dtl_id'], $saf_tax_id, $prop_dtl_id, $prop_tax_id, $created_on);
                        }
                    }
                    $saf_tax_list = $this->model_saf_tax->getsaftaxbysafId($level_dtl['saf_dtl_id']); // all
                    ...
                    $quartely_tax = $saf_tax_details['holding_tax'] + $saf_tax_details['water_tax'] + $saf_tax_details['education_cess'] + $saf_tax_details['health_cess'] + $saf_tax_details['latrine_tax'] + $saf_tax_details['additional_tax'];
                    $memo_type = 'SAM';
                    $genmemo_last_id = $this->model_saf_memo_dtl->generatesafmemono($level_dtl['saf_dtl_id'], $saf_tax_details['fy_mstr_id'], $saf_tax_details['qtr'], $saf_tax_details['arv'], $quartely_tax, $memo_type, $login_emp_details_id, $created_on, $previousPropDtl['id'], $previousPropDtl['new_holding_no']);
    ...
    public function uploadConsessionDocument($prop_dtl_id = NULL, $owner_id = NULL, $prop_doc_id = NULL)
    {
        ...
        if ($prop_dtl_id != NULL) {
            ...
            $data["sepcial_doc_dtl_grouped_by_owner"] = $this->model_prop_dtl->getSpecialDocOnwerGroupedDataPropDtlId($prop_dtl_id); //
            // $data['special_prop_value'] = $data["sepcial_doc_dtl_grouped_by_owner"]['special_prop_value'];
        }
        // print_var($data);
        // return;
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            if (isset($inputs["btn_upload"])) {
                ...
                $consession_doc_file = $this->request->getFile('handicapped_doc');
                // echo "doc file";
                // print_var($consession_doc_file);
                // return;
                $prop_special_doc_insert_id = $this->request->getFile('prop_doc_id');
                $newFileName = md5($prop_special_doc_insert_id);
                $file_ext = $consession_doc_file->getExtension();
                ...
                $consession_doc_path = $path . "/" . $newFileName . '.' . $file_ext;
                if ($consession_doc_file->move(WRITEPATH . 'uploads/' . $path . '/', $newFileName . '.' . $file_ext)) {

                    $inputUpdate = [
                        "prop_special_doc_tbl_id" => $prop_special_doc_insert_id,
                        "doc_path" => $consession_doc_path,
                    ];
                    $this->model_prop_dtl->uploadConsessionDocViaBackOffice($inputUpdate);
        ...
        // print_var($data);
        // return;
        return view('property/Saf/uploadConessionDocView', $data);
    }

    public function ConsessionDocUpload()
    {
        if ($this->request->getMethod() == 'post') {
          
            // $prop_dtl_id_raw['id'] = $this->model_prop_dtl->getIdFromMd5Id($prop_dtl_id);

            // print_var($this->request->getvar());
            // die;
            $ulb_mstr_id = $_SESSION['ulb_dtl']['ulb_mstr_id'];
            ...
            if ($gender_status == 1) {
                ...
                $rules = [
                    ...
                    'gender_doc_file' => 'uploaded[gender_doc_file]|max_size[gender_doc_file,5120]|ext_in[gender_doc_file,pdf]'

                ];
                if ($this->validate($rules)) {
                   

                    $female_doc_file = $this->request->getFile('gender_doc_file');
                    $prop_special_doc_insert_id = $this->request->getVar('gender_prop_special_doc_insert_id');
                    // echo "prp_doc_id ";
                    // echo $prop_special_doc_insert_id;
                    // die;
                    $newFileName = md5($prop_special_doc_insert_id);
            ...
            if ($dob_status == 1) {
                $rules = [
                    ...
                    'dob_doc_file' => 'uploaded[dob_doc_file]|max_size[dob_doc_file,5120]|ext_in[dob_doc_file,pdf]'

                ];
                if ($this->validate($rules)) {

                    $female_doc_file = $this->request->getFile('dob_doc_file');
                    $dob_special_doc_insert_id = $this->request->getVar('dob_prop_special_doc_insert_id');
                    $newFileName = md5($dob_special_doc_insert_id);
                    $file_ext = $female_doc_file->getExtension();
                    ...
                    if ($female_doc_file->move(WRITEPATH . 'uploads/' . $path . '/', $newFileName . '.' . $file_ext)) {
                        ...
                        $inputUpdate = [
                            "doc_path" => $female_doc_path,
                            "prop_special_doc_tbl_id" => $dob_special_doc_insert_id,
                        ];
                        $this->model_prop_dtl->uploadConsessionDocViaBackOffice($inputUpdate);
                        flashToast('message', 'Document Uploaded Successfully !');
                    }
            ...
            if ($handicapped_status == 1) {
                $rules = [
                    ...
                    'handicapped_doc_file' => 'uploaded[handicapped_doc_file]|max_size[handicapped_doc_file,5120]|ext_in[handicapped_doc_file,pdf]'

                ];
                if ($this->validate($rules)) {

                    $female_doc_file = $this->request->getFile('handicapped_doc_file');
                    $hadnicapped_prop_special_doc_insert_id = $this->request->getVar('handicapped_prop_special_doc_insert_id');
                    $newFileName = md5($hadnicapped_prop_special_doc_insert_id);
                    $file_ext = $female_doc_file->getExtension();
                    ...
                    if ($female_doc_file->move(WRITEPATH . 'uploads/' . $path . '/', $newFileName . '.' . $file_ext)) {
                        ...
                        $inputUpdate = [
                            "doc_path" => $female_doc_path,
                            "prop_special_doc_tbl_id" => $hadnicapped_prop_special_doc_insert_id,
                        ];
                        $this->model_prop_dtl->uploadConsessionDocViaBackOffice($inputUpdate);
                        flashToast('message', 'Document Uploaded Successfully !');
                    }
            ...
            if ($armed_status == 1) {
                $rules = [
                    ...
                    'armed_doc_file' => 'uploaded[armed_doc_file]|max_size[armed_doc_file,5120]|ext_in[armed_doc_file,pdf]'

                ];
                if ($this->validate($rules)) {

                    $female_doc_file = $this->request->getFile('armed_doc_file');
                    $armed_prop_special_doc_insert_id = $this->request->getVar('armed_prop_special_doc_insert_id');
                    $newFileName = md5($armed_prop_special_doc_insert_id);
                    $file_ext = $female_doc_file->getExtension();
                    ...
                    if ($female_doc_file->move(WRITEPATH . 'uploads/' . $path . '/', $newFileName . '.' . $file_ext)) {
                        ...
                        $inputUpdate = [
                            "doc_path" => $female_doc_path,
                            "prop_special_doc_tbl_id" => $armed_prop_special_doc_insert_id,
                        ];
                        $this->model_prop_dtl->uploadConsessionDocViaBackOffice($inputUpdate);
                        flashToast('message', 'Document Uploaded Successfully !');
                    }
        ...
        return $this->response->redirect(base_url('UploadConessionDocument/prop_inbox_index'));
    }
}
