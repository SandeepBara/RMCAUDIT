...
class WaterfieldSiteInspection extends AlphaController
{
	...
	public function field_verification($apply_water_conn_id)
	{
		...
		//$data['applicant_pay_details']=$this->Water_Transaction_Model->applicantPayDetails($data['connection_dtls']['id']);
		...
		if($this->request->getMethod()=='post'){
			...
			if(isset($_POST['set_si']))
			{
				...
				return $this->response->redirect(base_url('waterfieldSiteInspection/field_verification/'.$inputs['app_id'].''));
			}
			
			if(isset($_POST['update_si']))
			{
				$data = [
                        'uses_type_id' => $inputs['uses_type_id'],
                        'areasqft' => $inputs['areasqft'],
                        'related_id' => $inputs['related_id'],
						'new_pipeline' => $inputs['new_pipeline'],
						'pipelinesize' => $inputs['pipelinesize'],
						'pipe_type' => $inputs['pipe_type'],
						'is_regularization' => $inputs['is_regularization'],
                        'permissible_pipe_dia' => $inputs['permissible_pipe_dia'],
                        'permissible_pipe_qlty' =>$inputs['permissible_pipe_qlty'],
						'ferrule_size' => $inputs['ferrule_size'],
                        'road_type' => $inputs['road_type'],
                        'applicant_category' =>$inputs['applicant_category'],
						'created_on' =>date('Y-m-d H:i:s'),
					...
					$site_ins_arr['flat_count']=$inputs['flat_count'];
					...
					$site_ins_arr['ferrule_type_id']=$inputs['ferrule_type_id'];
					$site_ins_arr['road_type']=$inputs['road_type'];
					$site_ins_arr['road_type']=$inputs['road_type'];
					$site_ins_arr['created_on']=date('Y-m-d H:i:s');


				$data['SI_date_timeupdt']=$this->site_ins_model->SI_date_timeupdt($data);
