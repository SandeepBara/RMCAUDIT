...
use App\Models\model_view_saf_collection_report;
use App\Models\model_ward_mstr;

class SafCollectionWardWise extends AlphaController
{
    protected $db;
    protected $dbSystem;
    ...
    protected $model_view_saf_collection_report;
    ...
    public function __construct(){
        ...
        $this->model_view_saf_collection_report = new model_view_saf_collection_report($this->db);
    ...
    public function report()
    {
        ...
        if($this->request->getMethod()=='post')
        {
            ...
            if($ward_mstr_id!=""){
                $data['transactionList'] = $this->model_view_saf_collection_report->getAllTransactionWard($from_date,$to_date,$ward_mstr_id);
            }
            else{
                $data['transactionList'] = $this->model_view_saf_collection_report->getAllTransaction($from_date,$to_date);
            }
            ...
            $data['total'] = $total;
            return view('report/saf_collection_ward_wise_report',$data);
        }
        else
        {
            $from_date = date('Y-m-d');
            $data['transactionList'] = $this->model_view_saf_collection_report->getAllTransaction($from_date,$from_date);
               foreach ($data['transactionList'] as $key => $value){
                $total =$total+$value['payable_amt'];
                $prop_dtl_id = $value['prop_dtl_id'];
                $data['transactionList'][$key]['holding'] = $this->model_saf_dtl->getSafdetails($prop_dtl_id);
