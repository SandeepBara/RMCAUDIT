...
class WaterDocumentCitizen extends HomeController
{
    protected $db;
    ...
    public function index()
	{
        ...
        helper(['form']);
        $session = session();
        $ulb_dtl = $session->get('ulb_dtl')??getUlbDtl();
        ...
        if($this->request->getMethod()=='post')
        {
            ...
            return view('citizen/water/water_conn_list', $data);
        }
        else
        {
    ...
    public function doc_upload($id)
    {
        ...
        $water_conn_dtl=$data['water_conn_dtl'];
        ...
        foreach($data['owner_list'] as $key => $value)
        {
            ...
            $data['owner_list'][$key]['applicant_detail_id'] = $conn_owner_id['applicant_detail_id'];
            $data['owner_list'][$key]['conn_owner_id']=null;
            $data['owner_list'][$key]['conn_owner_doc']=null;
        ...
        if($this->request->getMethod()=='post' && $data['connection_dtls']['doc_verify_status']==0)
        {
            //print_var($_POST);die;
            $ulb_dtl['city'] = $ulb_city_nm;
            //Owner Document doc
            if(isset($_POST['btn_owner_doc']))
            {
                                        ...
                                        if($ownerinsert_last_idd = $this->model_applicant_doc->insertOwnerData($data))
                                        {
                                            if($owner_doc_file->IsValid() && !$owner_doc_file->hasMoved())
                                            {
                                                ...
                                                if($owner_doc_file->move(WRITEPATH.'uploads/'.$path.'/',$newFileNamee.'.'.$file_extt))
                                                {
                                                    ...
                                                    {
                                                        return $this->response->redirect(base_url('WaterDocumentCitizen/doc_upload/'.$id.''));
                                                    }
                                                }
                                            }
                                        }
                ...
                else
                {
                    ...
                    return view('citizen/water/water_document_upload', $data);
                }
            ...
            if(isset($_POST['btn_payment_receipt']))
            {
                ...
                $data = [
                        'apply_connection_id' => $apply_connection_id,
                        'doc_for' => 'HOLDING PROOF', //proof of holding or saf payment
                        'document_id' => 6, //Holding Receipt
                        'emp_details_id' => $login_emp_details_id,
                        'created_on' =>date('Y-m-d H:i:s')
                    ];
                $rules=[
            ...
            if (isset($inputs['btn_upload']))
			{
				...
				else
				{
                    ...
                    $data['err_msg']='Error Occurs!!';echo('hear error');
                    return view('citizen/water/water_document_upload', $data);
				}
			}
        }
        ...
        {
            flashToast("message", "Document is Already Verified");
            return $this->response->redirect(base_url('WaterDocumentCitizen/doc_upload/'.$id.''));
        }
        else
        {
            ...
            //print_var($data['water_doc_list']);die;
            $doc_count_uploded = 0;
            foreach($data['water_doc_list'] as $val)
            {
                if(is_array($val) && sizeof($val)>1)
                {
                    ...
                    {
                        $doc_for_test='';
                        $doc_for_test = $nested['doc_for'];
                        if($this->model_applicant_doc->check_doc_exist($apply_connection_id,$doc_for_test))
                        {
                            $doc_count_uploded=$doc_count_uploded+1;
                            break;
                        }
                    }
                }
            ...
            // print_var(sizeof($data['water_doc_list']));
            // print_var($doc_count_uploded);
            ...
            //print_var($data['connection_dtls']['elec_category']??'gggg');
            if($category=="BPL")
            {
                //print_var("hearkkkkkkkkkkkkk");die;
                ...
                if($data['owner_doc_upload_stts'] && (sizeof($data['water_doc_list'])==$doc_count_uploded))
                {
                    if($doc_status=='0')
                    { 
                        ...
                        if($payment_status=='1')
                        {
                            if($data['connection_dtls']['elec_category']??false && !empty($data['connection_dtls']['elec_category']) && $data['connection_dtls']['category']??false && !empty($data['connection_dtls']['category']))
                            {
                                $level_pending_insrt=$this->model_water_level_pending_dtl->insrtlevelpendingdtl($leveldata);
                                if(in_array($water_conn_dtl['apply_from'],['sws','swsc']))
                                {
                                    $sws_whare = ['apply_connection_id'=>$water_conn_dtl['id']];
                                    $sws = $this->Citizensw_water_model->getData($sws_whare);
                                    if(!empty($sws))
                                    {
                                        $sw = [];
                                        $sw['sw_stage']= 3 ;
                                        $where_sw = ['apply_connection_id'=>$water_conn_dtl['id'],'id'=> $sws['id']];                            
                                        $this->Citizensw_water_model->updateData($sw,$where_sw);
                                        
                                        $push_sw=array();
                                        $ulb_mstr_id = $_SESSION['ulb_dtl']['ulb_mstr_id']??getUlbDtl()['ulb_mstr_id']; 
                                        $path='';
                                        if($water_conn_dtl['apply_from']=='sws')
                                        {

                                            $push_sw['application_stage']=11;
                                            $push_sw['status']='Application sent to Dealing Assistant';
                                            $push_sw['acknowledgment_no']=$water_conn_dtl['application_no'];
                                            ...
                                            //print_var($resp);
                                            $respons_data=[];
                                            $respons_data['apply_connection_id']=$water_conn_dtl['id'];
                                        ...
                                        elseif($water_conn_dtl['apply_from']=='swsc')
                                        {
                                            ...
                                            if(isset($login['status']) && $login['status']=="Success")
                                            {
                                                $update_window_singin = [
                                                    "apply_connection_id" =>$water_conn_dtl['id'],
                                                ...
                                                $push_sw=array();
                                                $push_sw['application_stage']=3;
                                                $push_sw['current_status']='Application sent to Dealing Assistant';
                
                                                $push_sw['caf_no']=$sws['caf_no'];
                                                $push_sw['sws_reference_no']=$sws['department_id'];
                                                $push_sw['dept_reference_no']=$water_conn_dtl['application_no'];
                            ...
                            return $this->response->redirect(base_url('WaterDocumentCitizen/docview/'.$id.''));
                        }
                        else
                        {
                ...
                if($data['owner_doc_upload_stts'] && (sizeof($data['water_doc_list'])==$doc_count_uploded))
                {
                    ...
                    { 
                        ...
                        if($payment_status=='1')
                        {
                            ...
                            flashToast("message", "Document Updated successfully And Payment Done");
                            return $this->response->redirect(base_url('WaterDocumentCitizen/docview/'.$id.''));
                        }
                        else
                        {
                            
                            flashToast("message", "Document Updated successfully And Payment Pending");
                            return $this->response->redirect(base_url('WaterDocumentCitizen/docview/'.$id.''));
                        }
                    }
    ...
    public function view($id=null)
	{
        ...
        return view('citizen/water/water_conn_view', $data);
    ...
    public function docview($id=null)
	{
        ...
        return view('citizen/water/water_doc_view', $data);
    }
