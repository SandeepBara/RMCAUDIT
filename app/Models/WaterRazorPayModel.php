...
class WaterRazorPayModel extends Model
{
    ...
    public function UpdateRazorPayTable($data)
    {
        ...
        $param=[
                    ...
                    "related_id"=> $pg_request["related_id"],
        ...
        //print_r($pg_request);
        //print_r($param);
        $priv = $this->db->table('tbl_razor_pay_response')
                ->select('id')
                ->where("razor_pay_request_id",$data['pg_mas_id'])
                ->orderBy("id desc")
                ->get()
                ->getFirstRow("array");
        if(isset($priv['id']) && $priv['id'])
            return $priv['id'];
        $this->db->table('tbl_razor_pay_response')->insert($param);  
        return $this->db->insertID();
    }
    public function updataSatusReponse($id,$status=1)
    {
        $this->db->table('tbl_razor_pay_response')->where('id',$id)->update(['status'=>$status]);
