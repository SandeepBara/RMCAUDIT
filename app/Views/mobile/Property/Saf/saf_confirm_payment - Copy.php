...
<!--CONTENT CONTAINER-->
	...
	<div id="content-container">
		...
		<div id="page-content">
			...
			<form action="<?php echo base_url('mobisafDemandPayment/saf_confirm_payment/'.$id);?>" method="post">
				...
				if(!empty($demand_detail))
				{
						...
						<table class="table table-bordered" style="font-family:Arial, Helvetica, sans-serif; font-size:12px;">
							<tbody>
								...
								<tr>
									...
									<td>Rebet Amount of JSK/Online Payment</td>
					...
					<div class="panel">
						<div class="panel-body text-center">
							...
							<?php } ?>
								<a href="<?php echo base_url('mobisafDemandPayment/saf_due_details/'.$id);?>" type="button" class="btn btn-warning btn-labeled">View Demand Details</a>
								<a href="<?php echo base_url('mobisafDemandPayment/saf_property_details/'.$id);?>" type="button" class="btn btn-primary btn-labeled">View Property Details</a>
						</div>
					</div>
				</form>
			</div>
...
<script>
	...
	function calculate(){
		...
		for(i=1; i<=tlt_qtr; i++){
			...
			var penlty_mnth = (parseFloat(tol_mnth)-parseFloat(dif_qtr));
			//alert(totl_fy);
			if (totl_fy>=49) {
				var each_penlty = (parseFloat(totl_dmnd)/100)*(parseFloat(penlty_mnth));
				//alert(penlty_mnth);
				if(each_penlty>0){
		...
		//alert(tol_pent);
		var pay_amt = (parseFloat(total_dem)+parseFloat(late_fine)+parseFloat(tol_pent))-parseFloat(rebate);
...
$(document).ready(function(){
	...
	if(due_upto_year!=""){
		$.ajax({
			type: "POST",
			url: "<?php echo base_url('/mobisafDemandPayment/ajax_rungatequarter'); ?>",
			dataType:"json",
			data:
			{
				due_upto_year:due_upto_year,custm_id:custm_id
