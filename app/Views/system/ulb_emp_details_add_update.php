            ...
            <div class="col-sm-12">
                <div class="panel">
                    ...
                    <div class="panel-body">
                        <form class="form-horizontal" method="post" action="<?=base_url('UlbEmpDetails/ulbAddUpdate/');?><?=isset($id)?'/'.md5($id):"";?>" enctype="multipart/form-data">
                         ...
                         <div class="form-group">
                            <label class="col-sm-2">Upload Signature </label>
                            <div class="col-sm-3">
                                <span>Preferred Image : Maximum size of 1MB</span>
                                <input type="file" id="signature_path" name="signature_path" class="form-control m-t-xxs" accept=".png"/>
                                <input type="hidden" id="is_signature" name="is_signature" value="<?=(isset($signature_path))?($signature_path!="")?"is_image":"":"";?>" />
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-sm-2">&nbsp;</label>
                            <div class="col-sm-3">
                                <div class="div_photo_path_preview" style="width:100%; text-align:center;">
                                    <img id="signature_path_preview" src="<?=(isset($signature_path))?($signature_path!="")?base_url("/getImageLink.php?path=emp_signature/".$signature_path):base_url("/public/assets/img/avatar/default_avatar.png"):base_url("/public/assets/img/avatar/default_avatar.png");?>" alt="" style="height:230px; width:220px;"/>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            <label class="col-sm-2" >&nbsp;</label>
                            <div class="col-sm-3">
                                <input type="button" id="signature_path_remove" class="btn btn-dark btn-rounded btn-block" value="Remove Signature">
                            </div>
                        </div>
                        <div class="row">
                            <div class="form-group">
...
<script type="text/javascript">
    ...
    $("#signature_path").change(function() {
        ...
        if($.inArray(ext, ['png','jpg','jpeg']) == -1) {
            $("#signature_path").val("");
            $('#signature_path_preview').attr('src', "<?=base_url();?>/public/assets/img/avatar/default_avatar.png");
            ...
            if (input.files && input.files[0]) {
                ...
                reader.onload = function(e) {
                    $('#signature_path_preview').attr('src', e.target.result);
    ...
    $("#signature_path_remove").click(function(){
        $("#signature_path").val("");
        $('#signature_path_preview').attr('src', "<?=base_url();?>/public/assets/img/avatar/default_avatar.png");
        $("#is_signature").val("");
    });
...
<script type="text/javascript">
    ...
    if (session()->has('validation')) { ?>
        alert('<?php echo session()->get('validation')->getError('email_id') ?>');
        $("#email_id").css({"border-color":"red"});
        $("#email_id").focus();
    <?php }
    ?>
