...
<!--CONTENT CONTAINER-->
            ...
            <div id="content-container">
                <div id="page-head">
                    ...
                    <li class="active">ULB Employee Details List</li>
                    </ol>
                    <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
                    <!--End breadcrumb-->
                        ...
                        <div class="col-sm-12">
                            <div class="panel">
                                <div class="panel-heading">
                                    <h5 class="panel-title">ULB Employee Details List</h5>
                                </div>
                                <div class="panel-body">
                                     <div class="pad-btm">
                                        <a href="<?=base_url('UlbEmpDetails/ulbAddUpdate') ?>"><button id="demo-foo-collapse" class="btn btn-primary">Add New  <i class="fa fa-arrow-right"></i></button></a>
                                    </div>
                                    ...
                                    <?php else:
                                            $i=0;
                                            foreach ($ulbEmpList as $value):
                                    ?>
                                            <tr>
                                                <td><?=++$i;?></td>
                                                <td><?=$value['user_name']!=""?$value['user_name']:"";?></td>
                                                <td><?=$value['emp_name']!=""?$value['emp_name']:"";?></td>
                                                ...
                                                <td>
                                                    <?php if($value['lock_status']!=1){?>
                                                            <a class="btn btn-primary" href="<?php echo base_url('UlbEmpDetails/ulbAddUpdate/'.md5($value['id']));?>" role="button">Edit</a>
                                                        <?php }
                                                        else
                                                        {?> 
                                                            Services Lock
...
function lockfun(user_mstr_id){
    ...
    if(result)
     window.location.replace("<?=base_url();?>/UlbEmpDetails/lockEmployee/"+user_mstr_id);
}
function unlockfun(user_mstr_id){
    var result = confirm("Do You Want To Unlock Employee Services");
    if(result)
     window.location.replace("<?=base_url();?>/UlbEmpDetails/unlockEmployee/"+user_mstr_id);
}
