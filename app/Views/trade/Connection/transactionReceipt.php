...
<?php
	//echo(getDenialAmountTrade('2022-03-09 13:43:22','2022-03-09 13:43:23'));
	if(isset($header) && !empty($header))
	{
		echo $this->include("$header");
	}
	else
	{
  		echo $this->include('layout_vertical/popupHeader');
	}
?>
<style>
@media print {
											...
											<tr>
												<td>
													...
													<div style="border-bottom: #333333 dotted 2px; width: 565px; height: 20px; float: left; margin-left: 2px; padding-bottom: 3px; font-size: 14px; font-weight: bold; line-height: 18px;">
														&nbsp;
													 <?php echo ucwords(getIndianCurrency($transaction_details["paid_amount"]??0.00)); ?>
													&nbsp; Only
													</div>
												</td>
											</tr>
											<?php 
											if(in_array(strtoupper($transaction_details['payment_mode']),["ONLINE","CASH"]))											
											{ 
												?>
												<tr>
													<td height="35">
											...
											{ 
												?>
													<tr>
														<td height="35">
															<?php 
															if($transaction_details["payment_mode"]=="CHEQUE")
															{ 
																?>
																	<div style="float: left;">
