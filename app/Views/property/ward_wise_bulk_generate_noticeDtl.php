								...
								<table id="tbl_by_individual" class="table table-striped table-bordered" cellspacing="0" width="100%">
									<thead class="bg-trans-dark text-dark">
										<tr>
											<th id="selectAll">#</th>
                                            <th>Ward No</th> 
                                            <!-- <th>Total No. Of Defaulter</th> -->
                                            <th>Total Amount</th>
                                            <th>Total No. Defaulter</th>
                                            <th>Total No. Defaulter Payment Without Notice</th>
                                            <th>Total No. Of Notice Generated</th>
                                            <th>Total No. Of Notice Served</th>
                                            <th>Total No. Of Collection From Notice</th>							
										</tr>
									</thead>
									<tbody>
...
<script type="text/javascript">
   ...
   var individual_view_tbl = $('#tbl_by_individual').DataTable({
        ...
        'ajax': {
            "type": "POST",
            'url':'<?=base_url('propDtl/defaulterNoticesDtl');?>',            
        ...
        'columns': [
            ...
            // { 'data': 'total' },
            { 'data': 'balance' },
            { 'data': 'total_remaining_defaulter' },            
            { 'data': 'total_defaulter_pay_without_notice' },
            { 'data': 'total_notice_generated' },
            { 'data': 'total_notice_served' },
            { 'data': 'total_payment_received_from_notice' },
        ...
        drawCallback: function( settings )
        {
            try
            {
                ...
                $(api.column(2).footer() ).html(summary.balance??0);
                $(api.column(3).footer() ).html(summary.total_remaining_defaulter??0);
                $(api.column(4).footer() ).html(summary.total_defaulter_pay_without_notice??0);
                $(api.column(5).footer() ).html(summary.total_notice_generated??0);
                $(api.column(6).footer() ).html(summary.total_notice_served??0);
                $(api.column(7).footer() ).html(summary.total_payment_received_from_notice??0);
            }
            catch(err)
            {
    ...
    function callLoading(){
        $('#btn_search').click();
        history.replaceState(null, '', '<?=base_url("propDtl/defaulterNoticesDtl");?>');
    }
    ...
    function openWindow(element,type){
        ...
        url+="ajax=parent&";
        url = '<?=base_url("propDtl/generateBulkNoticeDtl")?>?'+url;
