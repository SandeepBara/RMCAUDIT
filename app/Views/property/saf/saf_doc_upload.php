...
<div id="content-container">
    <div id="page-head">
        ...
        <li><a href="#">SAF</a></li>
		<li><a href="<?=base_url();?>/safdtl/full/<?=md5($owner_detail[0]['saf_dtl_id']);?>">SAF Details</a></li>
    ...
    <div id="page-content">
								...
								<?php if($no_electric_connection=='t') { ?>
									<tr>
										...
										foreach ($no_electric_connection_doc_list as $key => $value) {
											if($key==0)
												echo $value["doc_name"].' (In case of No Electric Connection)';
											else
												echo ", ".$value["doc_name"];
										}
										?>
        ...
        if($show_rmc_btn==true)
        {
            ?>
            <div class="panel panel-bordered panel-dark">
                <div class="panel-body">
                    <div class="col-sm-2 col-sm-offset-5">
                        <a href="<?php echo base_url('safDoc/send_rmc/'.$saf_dtl_id);?>"><button data-target="#connection_modal" data-toggle="modal" class="btn btn-info btn-md">Send To ULB</button></a>
                    </div>
                </div>
            </div>
		    <?php 
...
$("#applicant_image_file").change(function() {
	var input = this;
	var ext = $(this).val().split('.').pop().toLowerCase();
	if($.inArray(ext, ['jpg','jpeg','png']) == -1) {
		$("#applicant_image_file").val("");
...
$("#applicant_doc_file").change(function() {
	var input = this;
	var ext = $(this).val().split('.').pop().toLowerCase();
	if($.inArray(ext, ['pdf']) == -1) {
		$("#applicant_doc_file").val("");
		alert('invalid document type');
	}
	if (input.files[0].size > 5242880) {
		$("#applicant_doc_file").val("");
...
$("#btn_owner_doc_upload").click(function(){
	proceed = true;  
	var applicant_image_file = $("#applicant_image_file").val();
	var owner_doc_mstr_id = $("#owner_doc_mstr_id").val();
	var applicant_doc_file = $("#applicant_doc_file").val();
	
	if(applicant_image_file=="")
	{
		alert("Please select applicant image");
		$("#applicant_image_file").css('border-color', 'red');
		return false;
	}
	...
	if(applicant_doc_file=="")
	{
		alert("Please select document");
		$("#applicant_doc_file").css('border-color', 'red');
		return false;
	}
...
$("#applicant_image_file").change(function(){ $(this).css('border-color',''); });
$("#owner_doc_mstr_id").change(function(){ $(this).css('border-color',''); });
$("#applicant_doc_file").change(function(){ $(this).css('border-color',''); });

$("#btn_upload").click(function(){
	proceed = true;  
	var upld_doc_path = $("#upld_doc_path").val();
	var upld_doc_mstr_id = $("#upld_doc_mstr_id").val();

	if(upld_doc_mstr_id=="")
	{
		alert("Please select document type");
		$("#upld_doc_mstr_id").css('border-color', 'red');
		return false;
	}
	if(upld_doc_path=="")
	{
		alert("Please select document");
		$("#upld_doc_path").css('border-color', 'red');
		return false;
	}
...
$("#upld_doc_path").change(function(){ $(this).css('border-color',''); });
$("#upld_doc_mstr_id").change(function(){ $(this).css('border-color',''); });
