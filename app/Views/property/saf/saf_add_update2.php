...
<style>
    #loading_text{
        animation: mv 0.5s linear infinite;
    }
    @keyframes mv {
        0%{
            color: red;
            ...
            <div class="panel panel-bordered panel-dark">
                ...
                <div class="panel-body">
                    ...
                    <div class="row">
                        ...
                        <input type="hidden" id="hasPrevOwnerDtlCount" name="hasPrevOwnerDtlCount" value="<?=$hasPrevOwnerDtlCount ?? 0;?>" />
                        <input type="hidden" id="isPrevPaymentCleared" name="isPrevPaymentCleared" value="<?=$isPrevPaymentCleared ?? 'NO';?>" />
                        ...
                        <div class="col-md-12">
                            <div class="table-responsive">
                                <table class="table table-bordered text-sm">
                                    ...
                                    <?php
                                    if(isset($prev_owner_name) && sizeof($prev_owner_name)>0)
                                    {
                                        for($i=0; $i<sizeof($prev_owner_name); $i++)
                                        {
                                            ?>
                                            <tr>
                                                <th><?=$prev_owner_name[$i];?>
                                                    <input type="hidden" id="prev_owner_name<?=$i;?>" name="prev_owner_name[]" class="prev_owner_name" value="<?=$prev_owner_name[$i];?>"></th>
                                                <th><?=$prev_guardian_name[$i];?>
                                                    <input type="hidden" id="prev_guardian_name<?=$i;?>" name="prev_guardian_name[]" class="prev_guardian_name" value="<?=$prev_guardian_name[$i];?>"></th>
                                                <th><?=$prev_relation_type[$i];?>
                                                    <input type="hidden" id="prev_relation_type<?=$i;?>" name="prev_relation_type[]" class="prev_relation_type" value="<?=$prev_relation_type[$i];?>"></th>
                                                <th><?=$prev_mobile_no[$i];?>
                                                    <input type="hidden" id="prev_mobile_no<?=$i;?>" name="prev_mobile_no[]" class="prev_mobile_no" value="<?=$prev_mobile_no[$i];?>"></th>
                                                <th><?=$prev_aadhar_no[$i];?>
                                                    <input type="hidden" id="prev_aadhar_no<?=$i;?>" name="prev_aadhar_no[]" class="prev_aadhar_no" value="<?=$prev_aadhar_no[$i];?>"></th>
                                                <th><?=$prev_pan_no[$i];?>
                                                    <input type="hidden" id="prev_pan_no<?=$i;?>" name="prev_pan_no[]" class="prev_pan_no" value="<?=$prev_pan_no[$i];?>"></th>
                                                <th><?=$prev_email[$i];?>
                                                    <input type="hidden" id="prev_email<?=$i;?>" name="prev_email[]" class="prev_email" value="<?=$prev_email[$i];?>"></th>
                                            </tr>
                                            <?php
                                        }
                                    }
                        ...
                        <div class="col-md-12">
                            <div class="table-responsive">
                                ...
                                ?>
                                        <tr>
                                            <td>
                                                <input maxlength="150" type="text" id="owner_name<?=$i+1;?>" name="owner_name[]" class="form-control owner_name" placeholder="Owner Name" value="<?=$owner_name[$i];?>" onkeypress="return isAlpha(event);" onkeyup="borderNormal(this.id);" />
                                            </td>
                                            <td>
                                                <input maxlength="150" type="text" id="guardian_name<?=$i+1;?>" name="guardian_name[]" class="form-control guardian_name" placeholder="Guardian Name" value="<?=$guardian_name[$i];?>" onkeypress="return isAlpha(event);" onkeyup="borderNormal(this.id);" />
                                            </td>
                                            <td>
                                ...
                                ?>
                                        <tr>
                                            <td>
                                                <input maxlength="150" type="text" id="owner_name1" name="owner_name[]" class="form-control owner_name" placeholder="Owner Name" value="" onkeypress="return isAlpha(event);" onkeyup="borderNormal(this.id);" />
                                            </td>
                                            <td>
                                                <input maxlength="150" type="text" id="guardian_name1" name="guardian_name[]" class="form-control guardian_name" placeholder="Guardian Name" value="" onkeypress="return isAlpha(event);" onkeyup="borderNormal(this.id);" />
                                            </td>
                                            <td>
    ...
    Underground Storage Area (in Sq. Ft.) <span class="text-danger">*</span></label>
            ...
            <div class="panel panel-bordered panel-dark">
                <div class="panel-body demo-nifty-btn text-center">
                    ...
                    <!-- <div id="loadingDiv2"  style="display: none; background: url(<?=base_url();?>/public/assets/img/loaders/transparent-background-loading.gif) no-repeat center center; position: relative; top: 0; left: 0; height: 100%; width: 100%; z-index: 9999999;"></div> -->
                </div>
            </div>
        </form>
...
<script type="text/javascript">
                        ...
                        if(data.response==true){
                            ...
                            for ( var i = 0; i < owner_dtl.length; i++ ) {
                                tbody += '<tr>';
                                    ...
                                    tbody += '<th>';
                                        var pan_no = "";
                                        if (owner_dtl[i]['pan_no']!=null) {
                                            pan_no = owner_dtl[i]['pan_no'];
                                        }
                                        tbody += pan_no;
                                        tbody += '<input type="hidden" id="prev_pan_no'+i+1+'" name="prev_pan_no[]" class="prev_pan_no" value="'+pan_no+'" />';
                                    tbody += '</th>';

                                    tbody += '<th>';
                                        var email = "";
                                        if (owner_dtl[i]['email']!=null) {
                                            email = owner_dtl[i]['email'];
                                        }
                                        tbody += email;
                                        tbody += '<input type="hidden" id="prev_email'+i+1+'" name="prev_email[]" class="prev_email" value="'+email+'" />';
                                    tbody += '</th>';
                                tbody += '</tr>';
                            ...
                            //added by hayat
                            var prop_dtl = data.prop_dtl;
                            $("#ward_mstr_id").val(prop_dtl.ward_mstr_id);
                            $("#new_ward_mstr_id").html(data.new_ward_list);
                            $("#new_ward_mstr_id").val(prop_dtl.new_ward_mstr_id);
                            $("#ownership_type_mstr_id").val(prop_dtl.ownership_type_mstr_id);
                            $("#prop_type_mstr_id").val(prop_dtl.prop_type_mstr_id);propTypeMstrChngFun();
                            $("#appartment_name").val(prop_dtl.appartment_name);
                            $("#flat_registry_date").val(prop_dtl.flat_registry_date);
                            $("#zone_mstr_id").val(prop_dtl.zone_mstr_id);
                            $("#road_type_mstr_id").val(prop_dtl.road_type_mstr_id);
                            $("#area_of_plot").val(prop_dtl.area_of_plot);
                            $("#prop_address").val(prop_dtl.prop_address);
                            $("#prop_city").val(prop_dtl.prop_city);

                            $("#prop_dist").val(prop_dtl.prop_dist);
                            $("#prop_state").val(prop_dtl.prop_state);
                            $("#prop_pin_code").val(prop_dtl.prop_pin_code);

                            $("#is_mobile_tower").val(0);
                            if(prop_dtl.is_mobile_tower=="t")
                            {
                                $("#is_mobile_tower").val(1);
                                $("#tower_area").val(prop_dtl.tower_area);
                                $("#tower_installation_date").val(prop_dtl.tower_installation_date);
                            }

                            $("#is_hoarding_board").val(0);
                            if(prop_dtl.is_hoarding_board=="t")
                            {
                                $("#is_hoarding_board").val(1);
                                $("#hoarding_area").val(prop_dtl.hoarding_area);
                                $("#hoarding_installation_date").val(prop_dtl.hoarding_installation_date);
                            }

                            $("#is_petrol_pump").val(0);
                            if(prop_dtl.is_petrol_pump=="t")
                            {
                                $("#is_petrol_pump").val(1);
                                $("#under_ground_area").val(prop_dtl.under_ground_area);
                                $("#petrol_pump_completion_date").val(prop_dtl.petrol_pump_completion_date);
                            }
                            
                            $("#is_water_harvesting").val((prop_dtl.is_water_harvesting=="t"?1:0));
                            $("#land_occupation_date").val(prop_dtl.occupation_date);

                            AddReAssessmentFloor(data.floor_dtl);
                        }
                        else
                        {
    ...
    function AddReAssessmentFloor(floor_dtl)
    {
        // by hayat
        var i=0;
        for (let x in floor_dtl)
        {
            ++i;
            floor = floor_dtl[x];
            //console.log(zf);
            if(i==1)
            {
                $("#floor_mstr_id"+zf+" option:contains(" + floor.floor_name + ")").attr('selected', 'selected');
                $("#usage_type_mstr_id"+zf+" option:contains(" + floor.usage_type + ")").attr('selected', 'selected');
                $("#occupancy_type_mstr_id"+zf+" option:contains(" + floor.occupancy_name + ")").attr('selected', 'selected');
                $("#const_type_mstr_id"+zf+" option:contains(" + floor.construction_type + ")").attr('selected', 'selected');
                $("#builtup_area"+zf+"").val(floor.builtup_area);
                $("#date_from"+zf+"").val(floor.date_from.slice(0, 7));
            }
            else
            {
                floor_dtl_append_fun();
                $("#floor_mstr_id"+zf+" option:contains(" + floor.floor_name + ")").attr('selected', 'selected');
                $("#usage_type_mstr_id"+zf+" option:contains(" + floor.usage_type + ")").attr('selected', 'selected');
                $("#occupancy_type_mstr_id"+zf+" option:contains(" + floor.occupancy_name + ")").attr('selected', 'selected');
                $("#const_type_mstr_id"+zf+" option:contains(" + floor.construction_type + ")").attr('selected', 'selected');
