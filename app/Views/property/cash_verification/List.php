...
<?php
$img=[1=> "correct.png", ""=> "incorrect.png", 0=> "incorrect.png"];

function checkVerified($todays_verified_collection, $emp_id){
    foreach($todays_verified_collection as $coll){
        if($coll["tran_by_emp_details_id"]==$emp_id)
        return $coll["total"];
    }
    return 0;
}
                                                ...
                                                ?>
                                                    <tr>
                                                        <?php if($value['total']>0){?>
                                                            ...
                                                            <td><?=$value['total'];?></td>
                                                            <td>
                                                                <a class="btn btn-primary" href="<?php echo base_url('CashVerification/View/'.$value['tran_by_emp_details_id'].'/'.$tran_date);?>">View</a>
                                                            </td>
                                                            <td><img src="<?=base_url();?>/public/assets/img/<?=$img[(checkVerified($todays_verified_collection, $value['tran_by_emp_details_id'])==$value['total'])];?>" class="imgg" /></td>
                                                            <td><?=checkVerified($todays_verified_collection, $value['tran_by_emp_details_id']);?></td>
                                                    <?php  } ?>
                                                    </tr>
                                                <?php 
                                            endforeach;
