    ...
    <div id="page-content">
        <div class="panel panel-mint">
            <div class="panel-heading">
                <h5 class="panel-title">SAF Collection Report With Rebate, Penalty</h5>
            </div>
                        ...
                        <div class="table-responsive">
                            <table id="empTable" class="table table-striped table-bordered text-sm">
                                ...
                                <tfoot>
                                    <tr class='bg-dark'>
                                        <th></th>
                                        <th style="color: white;">Total SAF</th>
                                        ...
                                        <th style="color: white;">0</th>
                                        <th style="color: white;">Total Penalty</th>
                                        <th style="color: white;">0</th>
                                    </tr>
                                </tfoot>
                            </table>
...
$(document).ready(function(){
    ...
    var dataTable = $('#empTable').DataTable({
        ...
        buttons: [
            'pageLength',
            {
                ...
                action: function ( e, dt, node, config ) {
                    ...
                    window.open('<?=base_url();?>/prop_report/SAFcollectionReportExcelExtra/'+gerUrl).opener = null;
                }
            }
        ],
        'ajax': {
            "type": "POST",
            'url':'<?=base_url('prop_report/SAFcollectionReportAjaxExtra');?>',
        ...
        'columns': [
            ...
            { 'data': 'rebate_amt' },
            { 'data': 'penalty_amt' },
            { 'data': 'late_penalty_amt' },
        ],
        drawCallback: function( settings ) {
            try {
                $("#footerResult").html(" (Total SAF - "+recordsTotal+", Total Collection - "+total_collection+", Total Rebate - "+total_rebate+" , Total Penalty - "+total_penalty+")");
                var api = this.api();
                $(api.column(2).footer() ).html(recordsTotal);
                $(api.column(8).footer() ).html(total_collection);
				$(api.column(10).footer() ).html(total_rebate);
                $(api.column(12).footer() ).html(total_penalty);
            } catch(err) {
...
function openPaymentModeWiseSummery()
{
   ...
   window.open('<?=base_url();?>/prop_report/SAFpaymentModeWiseSummery/'+gerUrl);
}
</script>
