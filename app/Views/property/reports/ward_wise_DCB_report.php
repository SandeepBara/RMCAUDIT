        ...
        <?php
            ...
            if (isset($report_list)) {
                ...
                foreach ($report_list as $list) {
                    ...
                    $total_advance_amount += $list['advance_amount']??0;
                    $total_remaining_advance_amount+=$list['remaining_advance_amount']??0;

                    $saf_arrear_coll += $list['saf_arrear_coll'] ?? 0;
                    $saf_current_coll += $list['saf_current_coll'] ?? 0;
                }

                $prop_current_balance =round($total_current_balance_amount - $total_advance_amount, 2);
                $prop_arrear_balance =round($total_arrear_balance_amount, 2);
                ...
                $topResult2 =   '<h5 class="panel-title" style="margin: 0px; padding: 0px;"><b>Current</b> (Total Demand - '.round($total_current_demand, 2).', Total Advance(Avg. Calculation) - '.round($total_advance_amount, 2).' , Total Collection - '.round($total_current_collection_amount, 2).', Total Balance - '.round($total_current_balance_amount-$total_advance_amount, 2).', <br> Total Saf Collection - '.(round($saf_current_coll, 2)).', Balance After Saf - '.(round($prop_current_balance - $saf_current_coll,0)).')</h5>
                                <h5 class="panel-title" style="margin: 0px; padding: 0px;"><b>Arrear</b> (Total Demand - '.round($total_arrear_demand, 2).', Total Collection - '.round($total_arrear_collection_amount, 2).', Total Balance - '.round($total_arrear_balance_amount, 2).', <br> Total Saf Collection - '.(round($saf_arrear_coll, 2)).', Balance After Saf - '.(round($total_arrear_balance_amount - $saf_arrear_coll,0)).')</h5>';
                            ...
                            <table id="dataTableID" class="table table-striped table-bordered text-sm">
                                ...
                                if (isset($report_list)) {
                                    ...
                                    { 
                                        ...
                                        <tr>
                                            ...
                                            <td><?=$list['collection_from_no_of_hh1'];?></td>
                                            <td><?=round($list['arrear_collection_amount']+$list['arrear_collection_amount2'],2);?></td>
                                            <!-- <td><?//=round($list['current_collection_amount'],2);?></td> -->
                                            <td><?=round($list['actual_collection_amount']+$list['actual_collection_amount2'],2);?></td>
                                            <td><?=round($list['total_collection_amount']+$list['total_collection_amount2'],2);?></td>
                                            <td><?=$list['balance_hh'];?></td>
                                            <td><?=round(abs($list['arrear_balance_amount']),2);?></td>
                                            <td><?=round(abs($list['current_balance_amount']),2);?></td>
                                            <td><?=round(abs($list['total_balance_amount2']),2);?></td>
                                            <td><?=$list['hh_percentage'];?></td>
                                            <td><?=$list['amount_percentage'];?></td>
                                            <td><?=$list['advance_amount'];?></td>
                                            <!-- <td><?=round(($list['remaining_advance_amount']??0),2);?></td> -->
                                ...
                                <tfoot>
                                    <tr class="bg-dark">
                                        ...
                                        <td><?=round($grand_total_demand??0, 2);?></td>
