    ...
    <div id="page-content">
            ...
            <div class="panel panel-bordered panel-dark">
                <div class="panel-heading">
                    <h3 class="panel-title"> Water Connection Details View
                    <?php
                    if($user_type==5)
                    {
                        ?>
                        <span class="btn btn-info pull-right"><a href="<?=base_url("WaterSearchApplicantsMobile/search_applicants_tc");?>"  style="color: white;">Back</a></span>
                        <?php
                    }
                    ?>
                    </h3>
							...
							if($owner_details)
							{
								foreach($owner_details as $val)
								{
									...
									<tr>
										...
										<td><?php echo $val['state'] ?? NULL;?></td>
										<td><?php echo $val['district'] ?? NULL;?></td>
										<td><?php echo $val['city'] ?? NULL;?></td>
									</tr>
									<?php
								}
							}
			...
			if($consumer_details['payment_status']==0 or isset($conn_fee_charge) or $penalty>0)
			{
					...
					<form method="post" action="<?php echo base_url('WaterPayment/proceed_payment');?>" id="myform">
						<div class="panel panel-bordered panel-dark">
							...
							<div class="table-responsive">
								<?php
								# Regularization
								if($consumer_details['connection_type_id']==2 && $consumer_details['payment_status']==0)
								{
								  ?>
									<p style="color: green; font-size: 15px; text-align: center; font-weight: bold;">
										Note: 10% rebate on penalty in case whole penalty is paid
									</p>
									<?php
								}
								?>
								...
								<table class="table table-bordered" style="font-size:14px;">
									<tr>
										...
										<input type="hidden" name="penalty" id="penalty" value="<?php echo ($penalty+$other_penalty);?>">
									  ...
									  <?php
										...
										if($consumer_details['connection_type_id']==2)
										{
										  ...
										  <tr>
											...
											<td>Pay Full<span class='text-danger'> *</span></td>
											<td>
											  <input type="radio" name="pay_full" id="pay_full" value="1" onclick="enable_installment(this.value)" required>Yes
											  <input type="radio" name="pay_full" id="pay_full" value="0" onclick="enable_installment(this.value)" required>No
                        ...
                        <option value="CARD">CARD</option>
		                    <option value="UPI">UPI</option>
									...
									<input type="hidden" name="connection_type_id" id="connection_type_id" value="<?php echo $consumer_details['connection_type_id'];?>">
									<input type="hidden" name="application_no" id="application_no" value="<?php echo $consumer_details['application_no'];?>">
