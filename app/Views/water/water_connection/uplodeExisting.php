...
<div id="content-container">
        ...
        { 
            ...
            <div id="page-head">
                ...
                <ol class="breadcrumb">
                    ...
                    <li class="active"><a href="#">Existing Consumer doc Uplode</a></li>
                </ol>
                <!--End breadcrumb-->
            </div>
            <?php 
    ...
    <div id="page-content">
		...
		<form id="form" name="form" method="post"  enctype="multipart/form-data" >
			...
			<div class="panel panel-bordered panel-dark" >
				...
				<div class="panel-body">
					...
					<div class="row" id ="address_block">
						...
						<div class="col-md-3 pad-btm">
                            <strong>
                                <?=isset($consumer_details['address'])?$consumer_details['address']:"N/A";?> 
                            </strong>
						</div>							
            ...
            <div class="panel panel-bordered panel-dark" >
                <div class="panel-heading"> 
                    <h3 class="panel-title">Uplode Document</h3>
                </div>
					...
					<div class="panel-body table-responsive">
						<table class="table table-bordered" style="font-family:Arial, Helvetica, sans-serif; font-size:12px;">
                            ...
                            <tbody>
                                <?php
                                $sl=1;
                            	foreach($uplodate_doc as $key=>$val)
                            	{ 
                                    $path = $val['document'];
                                    ...
                                    ?>                                   
                                        <tr>
                                            ...
                                            <td><?=date('d-m-Y',strtotime($val['creation_on']));?></td>
                                            <td><?=$val['emp_name'];?></td>
                                            <td>
                                                <?php
                                                if ($extention=="pdf")
            ...
            <div class="panel">
                <div class="panel-body">
                    <div class="row">
                        <label class="col-md-1">Document Type<span class="text-danger">*</span></label>
                        <div class="col-md-2 pad-btm">  
                            <select name="doc_for" id="doc_for" class="form-control" onchange="getdocument()">
                                <?php
                                foreach($documernt_list as $val)
                                {
                                    ?>
                                    <option value="<?=$val['doc_for']?>"><?=$val['doc_for']?></option>
                                    <?php
                                }
                                ?>
                            </select> 
                        </div>
                        <label class="col-md-1">Document Name<span class="text-danger">*</span></label>
                        <div class="col-md-2 pad-btm" id ='doc_list'>                               
...
<script>
    ...
    {
        var doc_for = $('#doc_for').val();        
        if(doc_for!='')
        {
            $.ajax({
                type:"POST",
                url: '<?php echo base_url("WaterUpdateConsumerConnectionMeterDoc/getdoc_list");?>',
                dataType:"json",
                data: {
                        "doc_for":doc_for
                },
                ...
                success:function(data){
                	...
                	{	
                		 ...
                		 var select="<select name='document_name' id='document_name' class='form-control'><option value=''>SELECT</option>";
                		 for(var k in data.result)
                		 {
                		 	 //console.log(k);
                		 	 select+="<option value='"+data.result[k]['id']+"'>"+data.result[k]['document_name']+"</option>";

                		 }
                		 select+="</select>";
                		 //alert(select);
                		 $("#doc_list").html(select);
                	}
                	else
                	{
                		$("#doc_list").text('Document Not Found');
                	}
                }
