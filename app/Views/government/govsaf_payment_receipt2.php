						...
						<div class="panel-body" id="print_watermark">
								...
								<tbody>
									...
									Account Description : Holding Tax &amp; Others</td>
										<td>
											...
											<?php if($basic_details["holding_no"]){?><div >Holding No. : &nbsp;<b><?=$basic_details["holding_no"];?></b></div><?php } ?>
										</td>
									</tr>
									...
									<tr>
										<td>Officer Name / Designation: &nbsp;
											<span style="font-size: 14px; font-weight: bold">
												<?php echo $basic_details["designation"]; ?> <?php if($basic_details["office_name"]) { echo "OF ". $basic_details["office_name"];} ?>
											</span>
										</td>
									</tr>
							...
							<table width="99%" border="1" align="center" cellpadding="0" cellspacing="0" style="font-family: Arial, Helvetica, sans-serif; font-size: 12px; text-align:center;">
								<tbody>
									...
									<?php if($coll_demand_dtl["holding_tax"]!=0){ ?>
									<tr>
										<td>Holding Tax</td>
										...
										<td><?=$coll_demand_dtl["holding_tax"];?></td>
									</tr>
									<?php } 
									if($coll_demand_dtl["additional_tax"]!=0){ ?>
									<tr>
										<td>RWH Penalty</td>
										<td><?=$tran_mode_dtl["from_qtr"];?></td>
										<td><?=$tran_mode_dtl["from_fyear"];?></td>
										<td><?=$tran_mode_dtl["upto_qtr"];?></td>
										<td><?=$tran_mode_dtl["upto_fyear"];?></td>
										<td><?=$coll_demand_dtl["additional_tax"];?></td>
									</tr>
									<?php } ?>
									
									<?php 
										$total_demand_amt = 0;
										$total_demand_amt += $coll_demand_dtl["holding_tax"];
										$total_demand_amt += $coll_demand_dtl["additional_tax"];
										if($penalty_dtl){ 
											foreach($penalty_dtl as $penalty_dtl_head) {
												//print_var($penalty_dtl_head);
												if ($penalty_dtl_head["value_add_minus"]=="Add") {
													if(($penalty_dtl_head["head_name"] !="Advance Amount") && ($penalty_dtl_head["head_name"] !="Adjusted Amount")){
														$total_demand_amt += $penalty_dtl_head["amount"];
													}
												} else if ($penalty_dtl_head["value_add_minus"]=="Minus") {
													$total_demand_amt -= $penalty_dtl_head["amount"];
												}
									?>
									<tr>
									...
									<tr>
										...
										<td><b><?=$total_demand_amt;?></b></td>
									</tr>
									<tr>
										<td colspan="5" style="text-align:right;"><b>Round Off Amount</b></td>
										<td><b><?=$tran_mode_dtl["round_off"];?></b></td>
									</tr>
									<?php if(isset($already_paid) && $already_paid>0) { ?>
									<tr>
										<td colspan="5" style="text-align:right;"><b>Already paid</b></td>
										<td><b><?=ROUND($already_paid).".00";?></b></td>
									</tr>
									<?php } ?>
									<tr>
									...
									<?php 
									$balance_amt = ($coll_demand_dtl["holding_tax"]+$coll_demand_dtl["additional_tax"])-$coll_dtl["holding_tax"];
									//$balance_amt = $total_demand_amt-$total_paid_amt;
									if(isset($already_paid) && $already_paid>0) {
										$balance_amt -= $already_paid;
									}
									if (round($balance_amt)>0) {
									?>
									<tr>
										<td colspan="5" style="text-align:right;">Balance Amount</td>
										<td><?=round($balance_amt, 2);?></td>
									</tr>
									<?php 
									}
									 ?>
